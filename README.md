...
正式源码与 manifest 将 SDK peer 精确锁定为当前发布线 `0.2.0`。公开包尚未发布时，可在主仓库先执行 `pnpm build:extension-tooling`，再使用本仓库的本地 SDK 链接脚本连接 Contract/SDK 发布投影；本地链接不作为正式 Release 的可取得性证据。

```powershell
pnpm install --config.auto-install-peers=false
pnpm link:local-sdk C:\path\to\glimmer-cradle
pnpm validate
$env:GCEX_ALLOW_DIRTY='1'; $env:GITHUB_REF_NAME='v0.2.0'; $env:GCEX_PLATFORMS='windows-x64,linux-x64'; pnpm release:prepare
```
